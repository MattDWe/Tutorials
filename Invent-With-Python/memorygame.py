"""
Memory Puzzle Game
Pick a box and match it with other boxes with the same symbol.
Github: MattDWe
"""
...
FPS = 30 #Limit frames per second
WINDOWWIDTH = 640 #Width of window in pixels
WINDOWHEIGHT = 480 #Height of the window in pixels
REVEALSPEED = 8 #Speed of boxes' sliding reveals and covers
BOXSIZE = 40 #Size of the box height and width in pixels
GAPSIZE = 10 #Number of pixels between boxes
...
def main():
    ...
    while True: #Main game loop
        ...
        for event in pygame.event.get(): #Event handling loop
            if event.type == QUIT or (event.type == KEYUP and event.key == K_EXCAPE):
                pygame.quit()
                sys.exit()
                ...
                if not revealedBoxes[boxx][boxy] and mouseClicked:
                    ...
                    if firstSelection == None: #Current box is first clicked
                        ...
                        if icon1shape != icon2shape or icon1color != icon2color:
                            #Icons do not match. Re-cover.
                            pygame.time.wait(1000) #1000 milliseconds = 1 sec
                            coverBoxesAnimation(mainBoard, [(firstSelection[0], firstSelection[1]), (boxx, boxy)])
                            revealedBoxes[firstSelection[0]][firstSelection [1]] = False
                            revealedBoxes[boxx][boxy] = False
                        elif hasWon(revealedBoxes): #Check if all pairs are found.
...
def hasWon(revealedBoxes):
    #Returns True if all the boxes been revealed.
